...
pop_data_2019 <- pop_data_2019 %>%
  ...
  covid_merged = inner_join(x = country_data,y = pop_data_2019, by = "country")
  covid_merged2 <- covid_merged[(covid_merged$tot_cases > -1),]
  
  covid_merged2 <- covid_merged2 %>% 
    mutate(normalized = cumulative_cases/SP.POP.TOTL)
```

```{r visualise, warning=FALSE}
ggplot(data = covid_merged2,
       mapping = aes(x = days_elapsed, 
                     y = normalized, 
                     color = country, 
                     label = end_label)) +
  ...
  geom_line(size = 0.7, alpha = 0.8) +
  # add points to line endings
  geom_point(data = covid_merged2 %>% filter(end_date)) +
  # add country labels, nudged above the lines
  geom_label_repel(nudge_y = 0.000001, direction = "y", hjust = 0.000001) + 
  # turn off legend
  guides(color = FALSE) +
  # use pretty colors
  scale_color_viridis_d() +
  ...
  labs(
    x = "Days since 10th confirmed death",
    y = "Normalized Cases",
    title = "Normalized Cases from COVID-19, selected countries",
    subtitle = glue("Data as of", as_of_date_formatted, .sep = " "),
    caption = "Source: github.com/RamiKrispin/coronavirus"
  )
```